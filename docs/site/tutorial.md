...
Now everything is ready, we can just start the engine.

~~~~ {#mycode .javascript}
    marioSearchEngine.addIndexes([index1, index2], function(res) {
        marioSearchEngine.open(fullproof.make_callback_caller(engineReady, true), fullproof.make_callback_caller(engineReady, false));
    });
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
